...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'catedria_app') THEN
    CREATE ROLE "catedria_app" WITH LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE
      NOINHERIT NOREPLICATION NOBYPASSRLS;
  ELSE
    -- Si ya existía, se le quitan los privilegios que anulan RLS. Que exista no
    -- garantiza que esté bien configurado.
    ALTER ROLE "catedria_app" WITH NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE;
  END IF;
END
$$;
...
DO $$
BEGIN
  EXECUTE format('GRANT CONNECT ON DATABASE %I TO %I', current_database(), 'catedria_app');
END
$$;
GRANT USAGE ON SCHEMA "public" TO "catedria_app";
GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA "public" TO "catedria_app";
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA "public" TO "catedria_app";
...
ALTER DEFAULT PRIVILEGES IN SCHEMA "public"
  GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO "catedria_app";
ALTER DEFAULT PRIVILEGES IN SCHEMA "public"
  GRANT USAGE, SELECT ON SEQUENCES TO "catedria_app";
...
BEGIN
  IF to_regclass('public._prisma_migrations') IS NOT NULL THEN
    REVOKE ALL ON TABLE "_prisma_migrations" FROM "catedria_app";
  END IF;
END
$$;
